...
| `ai_news.py` | **Morning-brief AI-news helper (2026-04-21).** Stdlib-only (`urllib` + `xml.etree` + `sqlite3`, zero pip deps) RSS/Atom fetcher with SHA-256 content-hash dedup in a local SQLite store, 48h recency window, 30-day prune. Feeds configurable; default set covers Simon Willison + HN AI-tag + arXiv cs.AI. Auto-marks returned items as seen; unreachable feeds skipped silently and surfaced in `feed_errors`. CLI: `fetch [--limit N]`, `stats`. Consumed by the `morning-brief` scheduled task's new `## AI news` section. A working copy lives in [`samples/scripts/ai_news.py`](samples/scripts/ai_news.py). |
...
| `To Do Questions.md` | heartbeat | Q&A tracker — **open blocks only**. Heartbeat posts questions; user answers inline; heartbeat picks up answers next cycle. Split from archive on 2026-04-21. |
