...
```mermaid
flowchart TB
    I["Input methods<br/>Keyboard · OS-level Whisper dictation<br/>(e.g. Wispr Flow — hotkey → transcribed text)"]
...
Input methods layer above entry points: text typed into any surface (CLI, desktop app, web) can come from a keyboard or from an OS-level Whisper dictation layer that types into whatever text field currently has focus. Three surfaces sit above the Claude session (entry points), three sit below (personas, routines, MCP). Each section from §2 onward details one slice.
